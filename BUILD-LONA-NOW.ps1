...
try {
    ...
    $HostUrl = 'https://github.com/seeynn/ios-build-host/releases/download/ios-host-latest/LonaHost-unsigned.ipa'
    try {
        Invoke-WebRequest -Uri $HostUrl -OutFile $HostIpa -UseBasicParsing
    } catch {
        Stop-WithMessage "The iPhone host is not published yet.`n`nOpen GitHub -> ios-build-host -> Actions -> iOS Host Build. Wait for the newest run to turn green, then double-click START-HERE-LONA.bat again."
    }
    if (-not (Test-Path $HostIpa) -or (Get-Item $HostIpa).Length -lt 1000000) { Stop-WithMessage 'The downloaded iPhone host is invalid.' }
